...
namespace SkyworthHMD
{
	...
	const FName FSkyworthHMD::OculusSystemName(TEXT("SkyworthHMD"));

	FName FSkyworthHMD::GetSystemName() const
		{
	...
	FString FSkyworthHMD::GetVersionString() const
	{
		...
		if (OVRP_FAILURE(FSkyworthHMDModule::GetPluginWrapper().GetVersion2(&Version)))
		{
			Version = "Unknown";
		}
	...
	bool FSkyworthHMD::DoesSupportPositionalTracking() const
	{
		ovrpBool trackingPositionSupported;
		return OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().GetTrackingPositionSupported2(&trackingPositionSupported)) && trackingPositionSupported;
	}


	bool FSkyworthHMD::HasValidTrackingPosition()
	{
		ovrpBool nodePositionTracked;
		return OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().GetNodePositionTracked2(ovrpNode_Head, &nodePositionTracked)) && nodePositionTracked;
	}
	...
	bool FSkyworthHMD::EnumerateTrackedDevices(TArray<int32>& OutDevices, EXRTrackedDeviceType Type)
	{
		CheckInGameThread();
		...
		for (uint32 TrackedDeviceId = 0; TrackedDeviceId < TrackedDeviceCount; TrackedDeviceId++)
		{
			if (Type == EXRTrackedDeviceType::Any || Type == TrackedDevices[TrackedDeviceId].Type)
			{
				...
				if (OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().GetNodePresent2(Node, &nodePresent)) && nodePresent)
				{
					const int32 ExternalDeviceId = SkyworthHMD::ToExternalDeviceId(Node);
	...
	void FSkyworthHMD::UpdateRTPoses()
	{
		CheckInRenderThread();
		...
		if (CurrentFrame)
		{
			...
			{
				FSkyworthHMDModule::GetPluginWrapper().Update3(ovrpStep_Render, CurrentFrame->FrameNumber, 0.0);
	...
	bool FSkyworthHMD::GetCurrentPose(int32 InDeviceId, FQuat& OutOrientation, FVector& OutPosition)
	{
		...
		ovrpNode Node = SkyworthHMD::ToOvrpNode(InDeviceId);
		...
		if (OVRP_FAILURE(FSkyworthHMDModule::GetPluginWrapper().GetNodePoseState3(ovrpStep_Render, CurrentFrame->FrameNumber, Node, &PoseState)) ||
		...
		//UE_LOG(LogHMD,Log,TEXT("FSkyworthHMD::GetCurrentPose Node(%d) OutOrientation=(%f,%f,%f,%f)"), Node, OutOrientation.X, OutOrientation.Y, OutOrientation.Z, OutOrientation.W);
		return true;
	}


	bool FSkyworthHMD::GetRelativeEyePose(int32 InDeviceId, EStereoscopicPass InEye, FQuat& OutOrientation, FVector& OutPosition)
	{
		OutOrientation = FQuat::Identity;
		OutPosition = FVector::ZeroVector;
		...
		if (OVRP_FAILURE(FSkyworthHMDModule::GetPluginWrapper().GetNodePoseState3(ovrpStep_Render, CurrentFrame->FrameNumber, ovrpNode_Head, &HmdPoseState)) ||
			OVRP_FAILURE(FSkyworthHMDModule::GetPluginWrapper().GetNodePoseState3(ovrpStep_Render, CurrentFrame->FrameNumber, Node, &EyePoseState)))
		{
			return false;
		}
		...
		OutPosition = EyePose.Position;//skyworth
		return true;
	}


	bool FSkyworthHMD::GetTrackingSensorProperties(int32 InDeviceId, FQuat& OutOrientation, FVector& OutPosition, FXRSensorProperties& OutSensorProperties)
	{
		CheckInGameThread();
		...
		if (OVRP_FAILURE(FSkyworthHMDModule::GetPluginWrapper().GetNodePoseState3(ovrpStep_Render, OVRP_CURRENT_FRAMEINDEX, Node, &PoseState)) || !ConvertPose(PoseState.Pose, Pose) ||
			OVRP_FAILURE(FSkyworthHMDModule::GetPluginWrapper().GetNodeFrustum2(Node, &Frustum)))
		{
			return false;
		}
	...
	void FSkyworthHMD::SetTrackingOrigin(EHMDTrackingOrigin::Type InOrigin)
	{
		...
		if (FSkyworthHMDModule::GetPluginWrapper().GetInitialized())
		{
			EHMDTrackingOrigin::Type lastOrigin = GetTrackingOrigin();
			FSkyworthHMDModule::GetPluginWrapper().SetTrackingOriginType2(ovrpOrigin);
	...
	EHMDTrackingOrigin::Type FSkyworthHMD::GetTrackingOrigin() const
	{
		EHMDTrackingOrigin::Type rv = EHMDTrackingOrigin::Eye;
		ovrpTrackingOrigin ovrpOrigin = ovrpTrackingOrigin_EyeLevel;

		if (FSkyworthHMDModule::GetPluginWrapper().GetInitialized() && OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().GetTrackingOriginType2(&ovrpOrigin)))
		{
			switch (ovrpOrigin)
			{
	...
	bool FSkyworthHMD::GetFloorToEyeTrackingTransform(FTransform& OutFloorToEye) const
	{
		float EyeHeight = 0.f;
		const bool bSuccess = FSkyworthHMDModule::GetPluginWrapper().GetInitialized() && OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().GetUserEyeHeight2(&EyeHeight));
		OutFloorToEye = FTransform( FVector(0.f, 0.f, -ConvertFloat_M2U(EyeHeight)) );

		return bSuccess;
	}

	void FSkyworthHMD::ResetOrientationAndPosition(float yaw)
	{
		Recenter(RecenterOrientationAndPosition, yaw);
	}

	void FSkyworthHMD::ResetOrientation(float yaw)
	{
		Recenter(RecenterOrientation, yaw);
	}

	void FSkyworthHMD::ResetPosition()
	{
		Recenter(RecenterPosition, 0);
	}

	void FSkyworthHMD::Recenter(FRecenterTypes RecenterType, float Yaw)
	{
		CheckInGameThread();
		...
		if (NextFrameToRender)
		{
			...
			ovrpPoseStatef poseState;
			FSkyworthHMDModule::GetPluginWrapper().Update3(ovrpStep_Render, NextFrameToRender->FrameNumber, 0.0);
			FSkyworthHMDModule::GetPluginWrapper().GetNodePoseState3(ovrpStep_Render, NextFrameToRender->FrameNumber, ovrpNode_Head, &poseState);
	...
	void FSkyworthHMD::SetBaseRotation(const FRotator& BaseRot)
	{
		SetBaseOrientation(BaseRot.Quaternion());
	}


	FRotator FSkyworthHMD::GetBaseRotation() const
	{
		return GetBaseOrientation().Rotator();
	}


	void FSkyworthHMD::SetBaseOrientation(const FQuat& BaseOrient)
	{
		CheckInGameThread();
	...
	FQuat FSkyworthHMD::GetBaseOrientation() const
	{
		CheckInGameThread();
	...
	bool FSkyworthHMD::IsHeadTrackingEnforced() const
	{
		if (IsInGameThread())
		{
	...
	void FSkyworthHMD::SetHeadTrackingEnforced(bool bEnabled)
	{
		CheckInGameThread();
	...
	bool FSkyworthHMD::IsHeadTrackingAllowed() const
	{
		CheckInGameThread();
		...
		bool allowed = FHeadMountedDisplayBase::IsHeadTrackingAllowed();
		//UE_LOG(LogHMD,Log,TEXT("FSkyworthHMD::IsHeadTrackingAllowed %d"), allowed);
		return allowed;
	}


	void FSkyworthHMD::OnBeginPlay(FWorldContext& InWorldContext)
	{
		CheckInGameThread();
	...
	void FSkyworthHMD::OnEndPlay(FWorldContext& InWorldContext)
	{
		CheckInGameThread();
	...
	{
		if (FSkyworthHMDModule::GetPluginWrapper().GetInitialized() == ovrpBool_False)
		{
			return;
		}
		...
		float valueFloat = 0;
		int valueInt = 0;
		if (OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().IsPerfMetricsSupported(ovrpPerfMetrics_App_CpuTime_Float, &bIsSupported)) && bIsSupported == ovrpBool_True)
		{
			if (OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().GetPerfMetricsFloat(ovrpPerfMetrics_App_CpuTime_Float, &valueFloat)))
			{
				SET_FLOAT_STAT(STAT_OculusSystem_AppCpuTime, valueFloat * 1000);
			}
		}
		if (OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().IsPerfMetricsSupported(ovrpPerfMetrics_App_GpuTime_Float, &bIsSupported)) && bIsSupported == ovrpBool_True)
		{
			if (OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().GetPerfMetricsFloat(ovrpPerfMetrics_App_GpuTime_Float, &valueFloat)))
			{
				SET_FLOAT_STAT(STAT_OculusSystem_AppGpuTime, valueFloat * 1000);
			}
		}
		if (OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().IsPerfMetricsSupported(ovrpPerfMetrics_Compositor_CpuTime_Float, &bIsSupported)) && bIsSupported == ovrpBool_True)
		{
			if (OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().GetPerfMetricsFloat(ovrpPerfMetrics_Compositor_CpuTime_Float, &valueFloat)))
			{
				SET_FLOAT_STAT(STAT_OculusSystem_ComCpuTime, valueFloat * 1000);
			}
		}
		if (OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().IsPerfMetricsSupported(ovrpPerfMetrics_Compositor_GpuTime_Float, &bIsSupported)) && bIsSupported == ovrpBool_True)
		{
			if (OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().GetPerfMetricsFloat(ovrpPerfMetrics_Compositor_GpuTime_Float, &valueFloat)))
			{
				SET_FLOAT_STAT(STAT_OculusSystem_ComGpuTime, valueFloat * 1000);
			}
		}
		if (OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().IsPerfMetricsSupported(ovrpPerfMetrics_Compositor_DroppedFrameCount_Int, &bIsSupported)) && bIsSupported == ovrpBool_True)
		{
			if (OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().GetPerfMetricsInt(ovrpPerfMetrics_Compositor_DroppedFrameCount_Int, &valueInt)))
			{
				SET_DWORD_STAT(STAT_OculusSystem_DroppedFrames, valueInt);
			}
		}
		if (OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().IsPerfMetricsSupported(ovrpPerfMetrics_System_GpuUtilPercentage_Float, &bIsSupported)) && bIsSupported == ovrpBool_True)
		{
			if (OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().GetPerfMetricsFloat(ovrpPerfMetrics_System_GpuUtilPercentage_Float, &valueFloat)))
			{
				SET_FLOAT_STAT(STAT_OculusSystem_GpuUtil, valueFloat * 100);
			}
		}
		if (OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().IsPerfMetricsSupported(ovrpPerfMetrics_System_CpuUtilAveragePercentage_Float, &bIsSupported)) && bIsSupported == ovrpBool_True)
		{
			if (OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().GetPerfMetricsFloat(ovrpPerfMetrics_System_CpuUtilAveragePercentage_Float, &valueFloat)))
			{
				SET_FLOAT_STAT(STAT_OculusSystem_CpuUtilAvg, valueFloat * 100);
			}
		}
		if (OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().IsPerfMetricsSupported(ovrpPerfMetrics_System_CpuUtilWorstPercentage_Float, &bIsSupported)) && bIsSupported == ovrpBool_True)
		{
			if (OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().GetPerfMetricsFloat(ovrpPerfMetrics_System_CpuUtilWorstPercentage_Float, &valueFloat)))
			{
				SET_FLOAT_STAT(STAT_OculusSystem_CpuUtilWorst, valueFloat * 100);
			}
		}
		if (OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().IsPerfMetricsSupported(ovrpPerfMetrics_Device_CpuClockFrequencyInMHz_Float, &bIsSupported)) && bIsSupported == ovrpBool_True)
		{
			if (OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().GetPerfMetricsFloat(ovrpPerfMetrics_Device_CpuClockFrequencyInMHz_Float, &valueFloat)))
			{
				SET_FLOAT_STAT(STAT_OculusSystem_CpuFreq, valueFloat);
			}
		}
		if (OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().IsPerfMetricsSupported(ovrpPerfMetrics_Device_GpuClockFrequencyInMHz_Float, &bIsSupported)) && bIsSupported == ovrpBool_True)
		{
			if (OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().GetPerfMetricsFloat(ovrpPerfMetrics_Device_GpuClockFrequencyInMHz_Float, &valueFloat)))
			{
				SET_FLOAT_STAT(STAT_OculusSystem_GpuFreq, valueFloat);
			}
		}
		if (OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().IsPerfMetricsSupported(ovrpPerfMetrics_Device_CpuClockLevel_Int, &bIsSupported)) && bIsSupported == ovrpBool_True)
		{
			if (OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().GetPerfMetricsInt(ovrpPerfMetrics_Device_CpuClockLevel_Int, &valueInt)))
			{
				SET_DWORD_STAT(STAT_OculusSystem_CpuClockLvl, valueInt);
			}
		}
		if (OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().IsPerfMetricsSupported(ovrpPerfMetrics_Device_GpuClockLevel_Int, &bIsSupported)) && bIsSupported == ovrpBool_True)
		{
			if (OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().GetPerfMetricsInt(ovrpPerfMetrics_Device_GpuClockLevel_Int, &valueInt)))
			{
	...
	bool FSkyworthHMD::OnStartGameFrame(FWorldContext& InWorldContext)
	{
		CheckInGameThread();
		...
		if (FSkyworthHMDModule::GetPluginWrapper().GetInitialized() &&
			OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().GetAppShouldRecreateDistortionWindow2(&appShouldRecreateDistortionWindow)) &&
...
//#endif
		...
		if (FSkyworthHMDModule::GetPluginWrapper().GetInitialized())
		{
			...
			FSkyworthHMDModule::GetPluginWrapper().GetAppHasVrFocus2(&bAppHasVRFocus);
			...
			if (OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().GetAppShouldQuit2(&AppShouldQuit)) && AppShouldQuit || OCFlags.EnforceExit)
			{
			...
			else if (OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().GetAppShouldRecenter2(&AppShouldRecenter)) && AppShouldRecenter)
			{
				...
				// Call FSkyworthHMDModule::GetPluginWrapper().RecenterTrackingOrigin2 to clear AppShouldRecenter flag
				FSkyworthHMDModule::GetPluginWrapper().RecenterTrackingOrigin2(ovrpRecenterFlag_IgnoreAll);
			}
...
//		if (FSkyworthHMDModule::GetPluginWrapper().GetMixedRealityInitialized())
//		{
//			FSkyworthHMDModule::GetPluginWrapper().UpdateExternalCamera();
//			FSkyworthHMDModule::GetPluginWrapper().UpdateCameraDevices();
//		}
//#endif
	...
	void FSkyworthHMD::DoSessionShutdown()
	{
		// Release resources
	...
	bool FSkyworthHMD::OnEndGameFrame(FWorldContext& InWorldContext)
	{
		CheckInGameThread();
	...
	bool FSkyworthHMD::IsHMDConnected()
	{
		CheckInGameThread();
	...
	bool FSkyworthHMD::IsHMDEnabled() const
	{
		CheckInGameThread();
	...
	EHMDWornState::Type FSkyworthHMD::GetHMDWornState()
	{
		ovrpBool userPresent;

		if (FSkyworthHMDModule::GetPluginWrapper().GetInitialized() && OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().GetUserPresent2(&userPresent)) && userPresent)
		{
	...
	void FSkyworthHMD::EnableHMD(bool enable)
	{
		CheckInGameThread();
	...
	bool FSkyworthHMD::GetHMDMonitorInfo(MonitorInfo& MonitorDesc)
	{
		return false;
	}


	void FSkyworthHMD::GetFieldOfView(float& InOutHFOVInDegrees, float& InOutVFOVInDegrees) const
	{
		ovrpFrustum2f Frustum;

		if (OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().GetNodeFrustum2(ovrpNode_EyeCenter, &Frustum)))
		{
	...
	void FSkyworthHMD::SetInterpupillaryDistance(float NewInterpupillaryDistance)
	{
		CheckInGameThread();

		if (FSkyworthHMDModule::GetPluginWrapper().GetInitialized())
		{
			FSkyworthHMDModule::GetPluginWrapper().SetUserIPD2(NewInterpupillaryDistance);
		}
	}


	float FSkyworthHMD::GetInterpupillaryDistance() const
	{
		CheckInGameThread();

		float UserIPD;

		if (!FSkyworthHMDModule::GetPluginWrapper().GetInitialized() || OVRP_FAILURE(FSkyworthHMDModule::GetPluginWrapper().GetUserIPD2(&UserIPD)))
		{
			return 0.0f;
		}
	...
	bool FSkyworthHMD::GetHMDDistortionEnabled(EShadingPath /* ShadingPath */) const
	{
		return false;
	}


	bool FSkyworthHMD::IsChromaAbCorrectionEnabled() const
	{
		CheckInGameThread();
	...
	bool FSkyworthHMD::HasHiddenAreaMesh() const
	{
	...
	bool FSkyworthHMD::HasVisibleAreaMesh() const
	{
		if (IsInRenderingThread())
		{
			if (ShouldDisableHiddenAndVisibileAreaMeshForSpectatorScreen_RenderThread())
	...
	void FSkyworthHMD::DrawHiddenAreaMesh_RenderThread(FRHICommandList& RHICmdList, EStereoscopicPass StereoPass) const
	{
		CheckInRenderThread();
	...
	void FSkyworthHMD::DrawVisibleAreaMesh_RenderThread(FRHICommandList& RHICmdList, EStereoscopicPass StereoPass) const
		{
		CheckInRenderThread();
	...
	float FSkyworthHMD::GetPixelDenity() const
	{
		CheckInGameThread();
		return Settings->PixelDensity;
	}

	void FSkyworthHMD::SetPixelDensity(const float NewPixelDensity)
	{
		CheckInGameThread();
		Settings->SetPixelDensity(NewPixelDensity);
	}

	FIntPoint FSkyworthHMD::GetIdealRenderTargetSize() const
	{
		CheckInGameThread();
		return Settings->RenderTargetSize;
	}

	bool FSkyworthHMD::IsStereoEnabled() const
	{
		if (IsInGameThread())
		{
	...
	bool FSkyworthHMD::IsStereoEnabledOnNextFrame() const
	{
		// !!!
	...
	bool FSkyworthHMD::EnableStereo(bool bStereo)
	{
		CheckInGameThread();
	...
	void FSkyworthHMD::AdjustViewRect(EStereoscopicPass StereoPass, int32& X, int32& Y, uint32& SizeX, uint32& SizeY) const
	{
		if (Settings.IsValid())
		{
	...
	void FSkyworthHMD::SetFinalViewRect(const enum EStereoscopicPass StereoPass, const FIntRect& FinalViewRect)
	{
		CheckInRenderThread();
	...
	void FSkyworthHMD::CalculateStereoViewOffset(const enum EStereoscopicPass StereoPassType, FRotator& ViewRotation, const float WorldToMeters, FVector& ViewLocation)
	{
	...
	FMatrix FSkyworthHMD::GetStereoProjectionMatrix(EStereoscopicPass StereoPassType) const
	{
		CheckInGameThread();
	...
	void FSkyworthHMD::InitCanvasFromView(FSceneView* InView, UCanvas* Canvas)
	{
	...
	void FSkyworthHMD::RenderTexture_RenderThread(class FRHICommandListImmediate& RHICmdList, class FRHITexture2D* BackBuffer, class FRHITexture2D* SrcTexture, FVector2D WindowSize) const
	{
		CheckInRenderThread();
		check(CustomPresent);
...
//#endif
	...
	FVector2D FSkyworthHMD::GetEyeCenterPoint_RenderThread(EStereoscopicPass StereoPassType) const
	{
		CheckInRenderThread();
	...
	FIntRect FSkyworthHMD::GetFullFlatEyeRect_RenderThread(FTexture2DRHIRef EyeTexture) const
	{
		CheckInRenderThread();
	...
	void FSkyworthHMD::CopyTexture_RenderThread(FRHICommandListImmediate& RHICmdList, FRHITexture2D* SrcTexture, FIntRect SrcRect, FRHITexture2D* DstTexture, FIntRect DstRect, bool bClearBlack, bool bNoAlpha) const
	{
		if (bClearBlack)
		{
	...
	bool FSkyworthHMD::PopulateAnalyticsAttributes(TArray<FAnalyticsEventAttribute>& EventAttributes)
	{
	...
	bool FSkyworthHMD::ShouldUseSeparateRenderTarget() const
	{
		return IsStereoEnabled();
	}


	void FSkyworthHMD::CalculateRenderTargetSize(const FViewport& Viewport, uint32& InOutSizeX, uint32& InOutSizeY)
	{
		CheckInGameThread();
	...
	void FSkyworthHMD::AllocateEyeBuffer()
	{
		CheckInGameThread();
	...
	bool FSkyworthHMD::NeedReAllocateViewportRenderTarget(const FViewport& Viewport)
	{
		CheckInGameThread();
	...
	bool FSkyworthHMD::NeedReAllocateDepthTexture(const TRefCountPtr<IPooledRenderTarget>& DepthTarget)
	{
		CheckInRenderThread();
	...
	bool FSkyworthHMD::NeedReAllocateFoveationTexture(const TRefCountPtr<IPooledRenderTarget>& FoveationTarget)
	{
		CheckInRenderThread();
	...
	bool FSkyworthHMD::AllocateRenderTargetTexture(uint32 Index, uint32 SizeX, uint32 SizeY, uint8 Format, uint32 NumMips, uint32 InTexFlags, uint32 InTargetableTextureFlags, FTexture2DRHIRef& OutTargetableTexture, FTexture2DRHIRef& OutShaderResourceTexture, uint32 NumSamples)
	{
	...
	bool FSkyworthHMD::AllocateDepthTexture(uint32 Index, uint32 SizeX, uint32 SizeY, uint8 Format, uint32 NumMips, uint32 FlagsIn, uint32 TargetableTextureFlags, FTexture2DRHIRef& OutTargetableTexture, FTexture2DRHIRef& OutShaderResourceTexture, uint32 NumSamples)
	{
		CheckInRenderThread();
	...
	bool FSkyworthHMD::AllocateFoveationTexture(uint32 Index, uint32 RenderSizeX, uint32 RenderSizeY, uint8 Format, uint32 NumMips, uint32 InTexFlags, uint32 InTargetableTextureFlags, FTexture2DRHIRef& OutTexture, FIntPoint& OutTextureSize)
	{
		CheckInRenderThread();
	...
	void FSkyworthHMD::UpdateViewportWidget(bool bUseSeparateRenderTarget, const class FViewport& Viewport, class SViewport* ViewportWidget)
	{
		CheckInGameThread();
	...
	FXRRenderBridge* FSkyworthHMD::GetActiveRenderBridge_GameThread(bool bUseSeparateRenderTarget)
	{
		CheckInGameThread();
	...
	void FSkyworthHMD::UpdateHMDWornState()
	{
	...
	uint32 FSkyworthHMD::CreateLayer(const IStereoLayers::FLayerDesc& InLayerDesc)
	{
		CheckInGameThread();
	...
	void FSkyworthHMD::DestroyLayer(uint32 LayerId)
	{
		CheckInGameThread();
	...
	void FSkyworthHMD::SetLayerDesc(uint32 LayerId, const IStereoLayers::FLayerDesc& InLayerDesc)
	{
		CheckInGameThread();
	...
	bool FSkyworthHMD::GetLayerDesc(uint32 LayerId, IStereoLayers::FLayerDesc& OutLayerDesc)
	{
		CheckInGameThread();
		FLayerPtr* LayerFound = LayerMap.Find(LayerId);
	...
	void FSkyworthHMD::MarkTextureForUpdate(uint32 LayerId)
	{
		CheckInGameThread();
		FLayerPtr* LayerFound = LayerMap.Find(LayerId);
	...
	void FSkyworthHMD::SetSplashRotationToForward()
	{
	...
	FSkyworthSplashDesc FSkyworthHMD::GetUESplashScreenDesc()
	{
	...
	void FSkyworthHMD::GetAllocatedTexture(uint32 LayerId, FTextureRHIRef &Texture, FTextureRHIRef &LeftTexture)
	{
	...
	IStereoLayers::FLayerDesc FSkyworthHMD::GetDebugCanvasLayerDesc(FTextureRHIRef Texture)
	{
	...
	void FSkyworthHMD::SetupViewFamily(FSceneViewFamily& InViewFamily)
	{
		CheckInGameThread();
	...
	void FSkyworthHMD::SetupView(FSceneViewFamily& InViewFamily, FSceneView& InView)
	{
		CheckInGameThread();
	}


	void FSkyworthHMD::BeginRenderViewFamily(FSceneViewFamily& InViewFamily)
	{
		CheckInGameThread();
	...
	void FSkyworthHMD::PreRenderViewFamily_RenderThread(FRHICommandListImmediate& RHICmdList, FSceneViewFamily& ViewFamily)
	{
		CheckInRenderThread();
	...
	void FSkyworthHMD::PreRenderView_RenderThread(FRHICommandListImmediate& RHICmdList, FSceneView& InView)
	{
	}


	void FSkyworthHMD::PostRenderViewFamily_RenderThread(FRHICommandListImmediate& RHICmdList, FSceneViewFamily& InViewFamily)
	{
		CheckInRenderThread();
	...
	int32 FSkyworthHMD::GetPriority() const
	{
	...
	bool FSkyworthHMD::IsActiveThisFrame(class FViewport* InViewport) const
	{
	...
	FSkyworthHMD::FSkyworthHMD(const FAutoRegister& AutoRegister)
	...
	FSkyworthHMD::~FSkyworthHMD()
	{
		Shutdown();
	}


	bool FSkyworthHMD::Startup()
	{
		if (GIsEditor)
		{
		...
		FCoreDelegates::ApplicationWillEnterBackgroundDelegate.AddRaw(this, &FSkyworthHMD::ApplicationPauseDelegate);
		FCoreDelegates::ApplicationHasEnteredForegroundDelegate.AddRaw(this, &FSkyworthHMD::ApplicationResumeDelegate);
#endif
	...
	void FSkyworthHMD::ApplicationPauseDelegate()
	{
		...
		/*ExecuteOnRenderThread([this]()
		{
			ExecuteOnRHIThread([this]()
			{*/
		UE_LOG(LogHMD, Log, TEXT("ApplicationPauseDelegate"));
		FSkyworthHMDModule::GetPluginWrapper().DestroyDistortionWindow2();
		/*	});
		});*/
		OCFlags.AppIsPaused = true;
	}

	void FSkyworthHMD::ApplicationResumeDelegate()
	{
		UE_LOG(LogHMD, Log, TEXT("ApplicationResumeDelegate"));
	...
	bool FSkyworthHMD::InitializeSession()
	{
		...
		if (!FSkyworthHMDModule::GetPluginWrapper().GetInitialized())
		{
			...
			if (OVRP_FAILURE(FSkyworthHMDModule::GetPluginWrapper().Initialize5(
		...
		FSkyworthHMDModule::GetPluginWrapper().SetAppEngineInfo2(
			"Unreal Engine",
			TCHAR_TO_ANSI(*FEngineVersion::Current().ToString()),
			GIsEditor ? ovrpBool_True : ovrpBool_False);

#if PLATFORM_ANDROID
		FSkyworthHMDModule::GetPluginWrapper().SetupDisplayObjects2(AndroidEGL::GetInstance()->GetRenderingContext()->eglContext, AndroidEGL::GetInstance()->GetDisplay(), AndroidEGL::GetInstance()->GetNativeWindow());
		ovrpBool mvSupport;
		FSkyworthHMDModule::GetPluginWrapper().GetSystemMultiViewSupported2(&mvSupport);
		...
		FSkyworthHMDModule::GetPluginWrapper().SetupDistortionWindow3(flag);
		FSkyworthHMDModule::GetPluginWrapper().SetSystemCpuLevel2(Settings->CPULevel);
		FSkyworthHMDModule::GetPluginWrapper().SetSystemGpuLevel2(Settings->GPULevel);
		FSkyworthHMDModule::GetPluginWrapper().SetTiledMultiResLevel((ovrpTiledMultiResLevel)Settings->FFRLevel);
		FSkyworthHMDModule::GetPluginWrapper().SetAppCPUPriority2(ovrpBool_True);
		FSkyworthHMDModule::GetPluginWrapper().SetReorientHMDOnControllerRecenter(Settings->Flags.bRecenterHMDWithController ? ovrpBool_True : ovrpBool_False);
	...
	void FSkyworthHMD::ShutdownSession()
	{
		ExecuteOnRenderThread([this]()
		{
			ExecuteOnRHIThread([this]()
		...
		FSkyworthHMDModule::GetPluginWrapper().Shutdown2();
	}

	bool FSkyworthHMD::InitDevice()
	{
		UE_LOG(LogHMD, Log, TEXT("FSkyworthHMD::InitDevice"));
		CheckInGameThread();
		...
		if (OVRP_FAILURE(FSkyworthHMDModule::GetPluginWrapper().GetSystemHeadsetType2(&Settings->SystemHeadset)))
		{
		...
		FSkyworthHMDModule::GetPluginWrapper().Update3(ovrpStep_Render, 0, 0.0);
...
#if !UE_BUILD_SHIPPING
		DrawDebugDelegateHandle = UDebugDrawService::Register(TEXT("Game"), FDebugDrawDelegate::CreateRaw(this, &FSkyworthHMD::DrawDebug));
#endif
	...
	void FSkyworthHMD::ReleaseDevice()
	{
		CheckInGameThread();
	...
	void BuildOcclusionMesh(FHMDViewMesh& Mesh, ovrpEye Eye, ovrpViewportStencilType MeshType)
	{
		...
		if (OVRP_FAILURE(Result = FSkyworthHMDModule::GetPluginWrapper().GetViewportStencil(Eye, MeshType, nullptr, &VertexCount, nullptr, &IndexCount)))
		{
			return;
		}
		...
		FSkyworthHMDModule::GetPluginWrapper().GetViewportStencil(Eye, MeshType, ovrpVertices, &VertexCount, pIndices, &IndexCount);
	...
	void FSkyworthHMD::SetupOcclusionMeshes()
	{
		CheckInGameThread();

		FSkyworthHMD* const Self = this;
	...
	void FSkyworthHMD::UpdateStereoRenderingParams()
	{
		CheckInGameThread();
		...
		if (Settings->Flags.bPixelDensityAdaptive)
		{
			...
			FSkyworthHMDModule::GetPluginWrapper().GetAdaptiveGpuPerformanceScale2(&AdaptiveGpuPerformanceScale);
		...
		*EyeLayerFound = MakeShareable(EyeLayer);
		...
		if (bIsUsingMobileMultiView)
		{
			...
			userDefinedSettings.stereoRenderingMode = SinglePassInstanced;
		}
		else 
		{
			userDefinedSettings.stereoRenderingMode = MultiPass;
			
		}
		FSkyworthHMDModule::GetPluginWrapper().SetUserDefinedSettings(userDefinedSettings);

#endif

		ovrpLayerDesc_EyeFov EyeLayerDesc;

		if (OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().CalculateEyeLayerDesc2(
			Layout,
			...
			&EyeLayerDesc)))
		{
			...
			ovrpRecti vpRect[2];
			FSkyworthHMDModule::GetPluginWrapper().CalculateEyeViewportRect(EyeLayerDesc, ovrpEye_Left, 1.0f, &vpRect[0]);
			FSkyworthHMDModule::GetPluginWrapper().CalculateEyeViewportRect(EyeLayerDesc, ovrpEye_Right, 1.0f, &vpRect[1]);
			...
			Settings->EyeProjectionMatrices[0] = EyeLayerDesc.ProjectionMatrix[0];//skyworthvr
			Settings->EyeProjectionMatrices[1] = EyeLayerDesc.ProjectionMatrix[1];//skyworthvr
			if (Frame.IsValid())
			{
	...
	void FSkyworthHMD::UpdateHmdRenderInfo()
	{
		CheckInGameThread();
		FSkyworthHMDModule::GetPluginWrapper().GetSystemDisplayFrequency2(&Settings->VsyncToNextVsync);
	}


	void FSkyworthHMD::InitializeEyeLayer_RenderThread(FRHICommandListImmediate& RHICmdList)
	{
		check(!InGameThread());
		CheckInRenderThread();
	...
	void FSkyworthHMD::ApplySystemOverridesOnStereo(bool force)
	{
		CheckInGameThread();
	...
	bool FSkyworthHMD::OnOculusStateChange(bool bIsEnabledNow)
	{
		if (!bIsEnabledNow)
		{
	...
	class FSceneViewport* FSkyworthHMD::FindSceneViewport()
	{
		if (!GIsEditor)
		{
	...
	bool FSkyworthHMD::ShouldDisableHiddenAndVisibileAreaMeshForSpectatorScreen_RenderThread() const
	{
		CheckInRenderThread();
	...
	ESpectatorScreenMode FSkyworthHMD::GetSpectatorScreenMode_RenderThread() const
	{
		CheckInRenderThread();
	...
	void FSkyworthHMD::DrawDebug(UCanvas* InCanvas, APlayerController* InPlayerController)
	{
		CheckInGameThread();
		...
		if (InCanvas && IsStereoEnabled() && Settings->Flags.bShowStats)
		{
			...
			if (OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().GetAppLatencyTimings2(&AppLatencyTimings)))
			{
				Y += RowHeight;
			...
			float UserIPD;
			if (OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().GetUserIPD2(&UserIPD)))
			{
	...
	bool FSkyworthHMD::IsHMDActive() const
	{
		return FSkyworthHMDModule::GetPluginWrapper().GetInitialized() != ovrpBool_False;
	}

	float FSkyworthHMD::GetWorldToMetersScale() const
	{
		CheckInGameThread();
	...
	FVector FSkyworthHMD::GetNeckPosition(const FQuat& HeadOrientation, const FVector& HeadPosition)
	{
		CheckInGameThread();
		...
		if (OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().GetUserNeckEyeDistance2(&NeckEyeDistance)))
		{
	...
	void FSkyworthHMD::SetBaseOffsetInMeters(const FVector& BaseOffset)
	{
		CheckInGameThread();
	...
	FVector FSkyworthHMD::GetBaseOffsetInMeters() const
	{
		CheckInGameThread();
	...
	bool FSkyworthHMD::ConvertPose(const ovrpPosef& InPose, FPose& OutPose) const
	{
		CheckInGameThread();
	...
	bool FSkyworthHMD::ConvertPose_RenderThread(const ovrpPosef& InPose, FPose& OutPose) const
	{
		CheckInRenderThread();
	...
	bool FSkyworthHMD::ConvertPose_Internal(const ovrpPosef& InPose, FPose& OutPose, const FSettings* Settings, float WorldToMetersScale)
	{
		return SkyworthHMD::ConvertPose_Internal(InPose, OutPose, Settings->BaseOrientation, Settings->BaseOffset, WorldToMetersScale);
	}


	FVector FSkyworthHMD::ScaleAndMovePointWithPlayer(ovrpVector3f& OculusHMDPoint)
	{
		CheckInGameThread();
	...
	ovrpVector3f FSkyworthHMD::WorldLocationToOculusPoint(const FVector& InUnrealPosition)
	{
		CheckInGameThread();
	...
	float FSkyworthHMD::ConvertFloat_M2U(float OculusFloat) const
	{
		CheckInGameThread();
	...
	FVector FSkyworthHMD::ConvertVector_M2U(ovrpVector3f OculusHMDPoint) const
	{
		CheckInGameThread();
	...
	bool FSkyworthHMD::GetUserProfile(UserProfile& OutProfile)
	{
		float UserIPD;
		...
		if (FSkyworthHMDModule::GetPluginWrapper().GetInitialized() &&
			OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().GetUserIPD2(&UserIPD)) &&
			OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().GetUserNeckEyeDistance2(&UserNeckEyeDistance)) &&
			OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().GetUserEyeHeight2(&UserEyeHeight)))
		{
	...
	float FSkyworthHMD::GetVsyncToNextVsync() const
	{
		CheckInGameThread();
	...
	FPerformanceStats FSkyworthHMD::GetPerformanceStats() const
	{
		return PerformanceStats;
	}

	void FSkyworthHMD::SetCPUAndGPULevel(int CPULevel, int GPULevel)
	{
		CheckInGameThread();
	...
	void FSkyworthHMD::SetFixedFoveatedRenderingLevel(FSFixedFoveatedRenderingLevel InFFRLevel)
	{
		CheckInGameThread();
		Settings->FFRLevel = InFFRLevel;
	}

	void FSkyworthHMD::SetColorScaleAndOffset(FLinearColor ColorScale, FLinearColor ColorOffset, bool bApplyToAllLayers)
	{
		CheckInGameThread();
	...
	bool FSkyworthHMD::DoEnableStereo(bool bStereo)
	{
		CheckInGameThread();
	...
	void FSkyworthHMD::ResetControlRotation() const
	{
	...
	FSettingsPtr FSkyworthHMD::CreateNewSettings() const
	{
	...
	FGameFramePtr FSkyworthHMD::CreateNewGameFrame() const
	{
	...
	void FSkyworthHMD::StartGameFrame_GameThread()
	{
		CheckInGameThread();
		check(Settings.IsValid());
		...
		if (!Frame.IsValid())
		{
			...
			if (!Splash->IsShown())
			{
				if (FSkyworthHMDModule::GetPluginWrapper().GetInitialized() && WaitFrameNumber != Frame->FrameNumber)
				{
					UE_LOG(LogHMD, Verbose, TEXT("FSkyworthHMDModule::GetPluginWrapper().WaitToBeginFrame %u"), Frame->FrameNumber);

					ovrpResult Result;
					if (OVRP_FAILURE(Result = FSkyworthHMDModule::GetPluginWrapper().WaitToBeginFrame(Frame->FrameNumber)))
					{
						UE_LOG(LogHMD, Error, TEXT("FSkyworthHMDModule::GetPluginWrapper().WaitToBeginFrame %u failed (%d)"), Frame->FrameNumber, Result);
					}
					else
					{
				...
				FSkyworthHMDModule::GetPluginWrapper().Update3(ovrpStep_Render, Frame->FrameNumber, 0.0);
			}
	...
	void FSkyworthHMD::FinishGameFrame_GameThread()
	{
		CheckInGameThread();
	...
	void FSkyworthHMD::StartRenderFrame_GameThread()
	{
		CheckInGameThread();
	...
	void FSkyworthHMD::FinishRenderFrame_RenderThread(FRHICommandListImmediate& RHICmdList)
	{
		CheckInRenderThread();
	...
	void FSkyworthHMD::StartRHIFrame_RenderThread()
	{
		CheckInRenderThread();
		...
		if (Frame_RenderThread.IsValid())
		{
			...
			ExecuteOnRHIThread_DoNotWait([this, XSettings, XFrame, XLayers]()
			{
				if (XFrame.IsValid())
				{
					...
					{
						UE_LOG(LogHMD, Verbose, TEXT("FSkyworthHMDModule::GetPluginWrapper().BeginFrame4 %u"), Frame_RHIThread->FrameNumber);						

						ovrpResult Result;
						if (OVRP_FAILURE(Result = FSkyworthHMDModule::GetPluginWrapper().BeginFrame4(Frame_RHIThread->FrameNumber, CustomPresent->GetOvrpCommandQueue())))
						{
							UE_LOG(LogHMD, Error, TEXT("FSkyworthHMDModule::GetPluginWrapper().BeginFrame4 %u failed (%d)"), Frame_RHIThread->FrameNumber, Result);
							Frame_RHIThread->ShowFlags.Rendering = false;
						}
						else
						{
#if PLATFORM_ANDROID
							FSkyworthHMDModule::GetPluginWrapper().SetTiledMultiResLevel((ovrpTiledMultiResLevel)Frame_RHIThread->FFRLevel);
#endif
						}
					}
				}
	...
	void FSkyworthHMD::FinishRHIFrame_RHIThread()
	{
		CheckInRHIThread();
		...
		if (Frame_RHIThread.IsValid())
		{
			...
			if (Frame_RHIThread->ShowFlags.Rendering && !Frame_RHIThread->Flags.bSplashIsShown)
			{
				...
				UE_LOG(LogHMD, Verbose, TEXT("FSkyworthHMDModule::GetPluginWrapper().EndFrame4 %u"), Frame_RHIThread->FrameNumber);

				ovrpResult Result;
				if (OVRP_FAILURE(Result = FSkyworthHMDModule::GetPluginWrapper().EndFrame4(Frame_RHIThread->FrameNumber, LayerSubmitPtr.GetData(), LayerSubmitPtr.Num(), CustomPresent->GetOvrpCommandQueue())))
				{
					UE_LOG(LogHMD, Error, TEXT("FSkyworthHMDModule::GetPluginWrapper().EndFrame4 %u failed (%d)"), Frame_RHIThread->FrameNumber, Result);
				}
				else
				{
...
#define BOOLEAN_COMMAND_HANDLER_BODY(ConsoleName, FieldExpr)\
	...
	void FSkyworthHMD::UpdateOnRenderThreadCommandHandler(const TArray<FString>& Args, UWorld* World, FOutputDevice& Ar)
	{
		CheckInGameThread();
	...
	void FSkyworthHMD::PixelDensityMinCommandHandler(const TArray<FString>& Args, UWorld*, FOutputDevice& Ar)
	{
		CheckInGameThread();
	...
	void FSkyworthHMD::PixelDensityMaxCommandHandler(const TArray<FString>& Args, UWorld*, FOutputDevice& Ar)
	{
		CheckInGameThread();
	...
	void FSkyworthHMD::HQBufferCommandHandler(const TArray<FString>& Args, UWorld*, FOutputDevice& Ar)
	{
		CheckInGameThread();
	...
	void FSkyworthHMD::HQDistortionCommandHandler(const TArray<FString>& Args, UWorld*, FOutputDevice& Ar)
	{
		CheckInGameThread();
	...
	void FSkyworthHMD::ShowGlobalMenuCommandHandler(const TArray<FString>& Args, UWorld* World, FOutputDevice& Ar)
	{
		CheckInGameThread();

		if (!OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().ShowSystemUI2(ovrpUI::ovrpUI_GlobalMenu)))
		{
			Ar.Logf(TEXT("Could not show platform menu"));
		}
	}

	void FSkyworthHMD::ShowQuitMenuCommandHandler(const TArray<FString>& Args, UWorld* World, FOutputDevice& Ar)
	{
		CheckInGameThread();

		if (!OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().ShowSystemUI2(ovrpUI::ovrpUI_ConfirmQuit)))
		{
			Ar.Logf(TEXT("Could not show platform menu"));
		}
	}
...
#if !UE_BUILD_SHIPPING
	void FSkyworthHMD::StatsCommandHandler(const TArray<FString>& Args, UWorld*, FOutputDevice& Ar)
	{
		CheckInGameThread();
	...
	void FSkyworthHMD::ShowSettingsCommandHandler(const TArray<FString>& Args, UWorld* World, FOutputDevice& Ar)
	{
		Ar.Logf(TEXT("stereo ipd=%.4f\n nearPlane=%.4f"), GetInterpupillaryDistance(), GNearClippingPlane);
	}


	void FSkyworthHMD::IPDCommandHandler(const TArray<FString>& Args, UWorld* World, FOutputDevice& Ar)
	{
		if (Args.Num() > 0)
		{
	...
	void FSkyworthHMD::LoadFromSettings()
	{
		USkyworthHMDRuntimeSettings* HMDSettings = GetMutableDefault<USkyworthHMDRuntimeSettings>();
		check(HMDSettings);
