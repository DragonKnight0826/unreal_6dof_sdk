...
class FSkyworthHMD : public FHeadMountedDisplayBase, public FXRRenderTargetManager, public IStereoLayers, public FSceneViewExtensionBase, public FSkyworthAssetManager
{
	friend class USkyworthFunctionLibrary;
	friend FSkyworthHMDModule;
...
public:
	FSkyworthHMD(const FAutoRegister&);
	~FSkyworthHMD();
...
protected:
	...
	FSkyworthSplashDesc GetUESplashScreenDesc();
...
public:
	...
	*/
	...
	SKYWORTHHMD_API bool ConvertPose(const ovrpPosef& InPose, FPose& OutPose) const;
	SKYWORTHHMD_API bool ConvertPose_RenderThread(const ovrpPosef& InPose, FPose& OutPose) const;
	SKYWORTHHMD_API static bool ConvertPose_Internal(const ovrpPosef& InPose, FPose& OutPose, const FSettings* Settings, float WorldToMetersScale = 100.0f);
	...
	SKYWORTHHMD_API void StartGameFrame_GameThread(); // Called from OnStartGameFrame or from FOculusInput::SendControllerEvents (first actual call of the frame)
	...
	void SetFixedFoveatedRenderingLevel(FSFixedFoveatedRenderingLevel InFFRLevel);
	void SetColorScaleAndOffset(FLinearColor ColorScale, FLinearColor ColorOffset, bool bApplyToAllLayers);

	SKYWORTHHMD_API void UpdateRTPoses();
...
typedef TSharedPtr< FSkyworthHMD, ESPMode::ThreadSafe > FSkyworthHMDPtr;
