...
FCustomPresent::FCustomPresent(class FSkyworthHMD* InOculusHMD, ovrpRenderAPIType InRenderAPI, EPixelFormat InDefaultPixelFormat, bool bInSupportsSRGB)
...
void FCustomPresent::ReleaseResources_RHIThread()
{
	...
	{
		FSkyworthHMDModule::GetPluginWrapper().DestroyMirrorTexture2();
...
void FCustomPresent::UpdateMirrorTexture_RenderThread()
{
	...
	if (FSkyworthHMDModule::GetPluginWrapper().GetInitialized())
	{
			...
			MirrorWindowSize.X != 0 && MirrorWindowSize.Y != 0)
		{
			...
			ExecuteOnRHIThread([&]()
			{
				FSkyworthHMDModule::GetPluginWrapper().SetupMirrorTexture2(GetOvrpDevice(), Height, Width, GetDefaultOvrpTextureFormat(), &TextureHandle);
			});
...
void FCustomPresent::FinishRendering_RHIThread()
{
	...
	if (OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().GetGPUFrameTime(&GPUFrameTime)))
	{
...
int FCustomPresent::GetSystemRecommendedMSAALevel() const
{
	...
	FSkyworthHMDModule::GetPluginWrapper().GetSystemRecommendedMSAALevel2(&SystemRecommendedMSAALevel);
