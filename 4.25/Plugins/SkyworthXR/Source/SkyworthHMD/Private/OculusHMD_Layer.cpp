...
FOvrpLayer::~FOvrpLayer()
{
	if (!IsInGameThread())
	{
		...
		{
			FSkyworthHMDModule::GetPluginWrapper().DestroyLayer(OvrpLayerId);
		});
	}
}
...
void FLayer::Initialize_RenderThread(const FSettings* Settings, FCustomPresent* CustomPresent, FRHICommandListImmediate& RHICmdList, const FLayer* InLayer)
{
	...
	else
	{
		...
		FSkyworthHMDModule::GetPluginWrapper().CalculateLayerDesc(
			Shape,
	...
	else
	{
		...
		ExecuteOnRHIThread([&]()
		{
			// UNDONE Do this in RenderThread once OVRPlugin allows FSkyworthHMDModule::GetPluginWrapper().SetupLayer to be called asynchronously
			int32 TextureCount;
			if (OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().SetupLayer(CustomPresent->GetOvrpDevice(), OvrpLayerDesc.Base, (int*) &OvrpLayerId)) &&
				OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().GetLayerTextureStageCount(OvrpLayerId, &TextureCount)))
			{
				// Left
				{
					...
					for (int32 TextureIndex = 0; TextureIndex < TextureCount; TextureIndex++)
					{
						...
						if (!OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().GetLayerTexture2(OvrpLayerId, TextureIndex, ovrpEye_Left, &ColorTextures[TextureIndex], DepthTexHdlPtr)))
						{
						...
						if (bValidFoveationTextures)
						{
							...
							if (!OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().GetLayerTextureFoveation(OvrpLayerId, TextureIndex, ovrpEye_Left, &FoveationTextures[TextureIndex], &FoveationTextureSize)) || 
				...
				if(OvrpLayerDesc.Layout == ovrpLayout_Stereo)
				{
					...
					for (int32 TextureIndex = 0; TextureIndex < TextureCount; TextureIndex++)
					{
						...
						if (!OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().GetLayerTexture2(OvrpLayerId, TextureIndex, ovrpEye_Right, &RightColorTextures[TextureIndex], DepthTexHdlPtr)))
						{
