...
FSplash::FSplash(FSkyworthHMD* InOculusHMD) :
...
void FSplash::LoadSettings()
{
	...
	check(HMDSettings);
	ClearSplashes();
	for (const FSkyworthSplashDesc& SplashDesc : HMDSettings->SplashDescs)
	{
...
{
	UE_LOG(LogHMD, Log, TEXT("FSplash::Startup"));
	CheckInGameThread();
	...
	if (!bInitialized)
	{
		...
		if (OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().GetSystemDisplayFrequency2(&SystemDisplayFrequency)))
		{
...
void FSplash::RenderFrame_RenderThread(FRHICommandListImmediate& RHICmdList)
{
	...
	ovrpResult Result;
	if ( FSkyworthHMDModule::GetPluginWrapper().GetInitialized() && OculusHMD->WaitFrameNumber != Frame->FrameNumber)
	{ 
		UE_LOG(LogHMD, Verbose, TEXT("Splash FSkyworthHMDModule::GetPluginWrapper().WaitToBeginFrame %u"), XFrame->FrameNumber);
		if (OVRP_FAILURE(Result = FSkyworthHMDModule::GetPluginWrapper().WaitToBeginFrame(XFrame->FrameNumber)))
		{
			UE_LOG(LogHMD, Error, TEXT("Splash FSkyworthHMDModule::GetPluginWrapper().WaitToBeginFrame %u failed (%d)"), XFrame->FrameNumber, Result);
	...
	{
		if (OVRP_FAILURE(Result = FSkyworthHMDModule::GetPluginWrapper().Update3(ovrpStep_Render, XFrame->FrameNumber, 0.0)))
		{
			UE_LOG(LogHMD, Error, TEXT("Splash FSkyworthHMDModule::GetPluginWrapper().Update3 %u failed (%d)"), XFrame->FrameNumber, Result);
		}
	}
	...
	ExecuteOnRHIThread_DoNotWait([this, XSettings, XFrame, XLayers]()
	{
		...
		if (XFrame->ShowFlags.Rendering)
		{
			UE_LOG(LogHMD, Verbose, TEXT("Splash FSkyworthHMDModule::GetPluginWrapper().BeginFrame4 %u"), XFrame->FrameNumber);
			if (OVRP_FAILURE(ResultT = FSkyworthHMDModule::GetPluginWrapper().BeginFrame4(XFrame->FrameNumber, CustomPresent->GetOvrpCommandQueue())))
			{
				UE_LOG(LogHMD, Error, TEXT("Splash FSkyworthHMDModule::GetPluginWrapper().BeginFrame4 %u failed (%d)"), XFrame->FrameNumber, ResultT);
				XFrame->ShowFlags.Rendering = false;
			}
		}
		...
		if (XFrame->ShowFlags.Rendering)
		{
			...
			UE_LOG(LogHMD, Verbose, TEXT("Splash FSkyworthHMDModule::GetPluginWrapper().EndFrame4 %u"), XFrame->FrameNumber);
			if (OVRP_FAILURE(ResultT = FSkyworthHMDModule::GetPluginWrapper().EndFrame4(XFrame->FrameNumber, LayerSubmitPtr.GetData(), LayerSubmitPtr.Num(), CustomPresent->GetOvrpCommandQueue())))
			{
				UE_LOG(LogHMD, Error, TEXT("Splash FSkyworthHMDModule::GetPluginWrapper().EndFrame4 %u failed (%d)"), XFrame->FrameNumber, ResultT);
			}
			else
			{
				for (int32 LayerIndex = 0; LayerIndex < Layers_RHIThread.Num(); LayerIndex++)
...
int FSplash::AddSplash(const FSkyworthSplashDesc& Desc)
{
	CheckInGameThread();
...
{
	FSkyworthSplashDesc OculusDesc;
...
{
	UE_LOG(LogHMD, Log, TEXT("FSplash::ClearSplashes"));
	CheckInGameThread();
...
bool FSplash::GetSplash(unsigned InSplashLayerIndex, FSkyworthSplashDesc& OutDesc)
{
	CheckInGameThread();
...
IStereoLayers::FLayerDesc FSplash::StereoLayerDescFromOculusSplashDesc(FSkyworthSplashDesc OculusDesc)
{
	IStereoLayers::FLayerDesc LayerDesc;
...
void FSplash::DoShow()
{
...

	{
		...
		FSkyworthSplashDesc UESplashDesc = OculusHMD->GetUESplashScreenDesc();
