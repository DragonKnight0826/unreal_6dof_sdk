...
DEFINE_LOG_CATEGORY(LogSkyworthPluginWrapper);

static void* LoadEntryPoint(void* handle, const char* EntryPointName);

bool InitializeSkyworthPluginWrapper(SkyworthPluginWrapper* wrapper)
{
	if (wrapper->Initialized)
	{
		UE_LOG(LogSkyworthPluginWrapper, Warning, TEXT("wrapper already initialized"));
		return true;
	}
	...
	if (VersionValid)
	{
		LibraryHandle = FSkyworthHMDModule::GetOVRPluginHandle();
		if (LibraryHandle == nullptr)
		{
			UE_LOG(LogSkyworthPluginWrapper, Warning, TEXT("GetOVRPluginHandle() returned NULL"));
			return false;
		}
	}
	else
...
#define OCULUS_BIND_ENTRY_POINT(Func)	{ "ovrp_"#Func, (void**)&wrapper->Func }
	...
	SkyworthEntryPoint entryPointArray[] =
	{
		// OVR_Plugin.h
		...
		OCULUS_BIND_ENTRY_POINT(GetLocalTrackingSpaceRecenterCount),//159
		OCULUS_BIND_ENTRY_POINT(GetKeyStatue),//159
		...
		OCULUS_BIND_ENTRY_POINT(SetUserDefinedSettings),
...
#undef OCULUS_BIND_ENTRY_POINT
	...
	for (int i = 0; i < UE_ARRAY_COUNT(entryPointArray); ++i)
	{
		*(entryPointArray[i].EntryPointPtr) = LoadEntryPoint(LibraryHandle, entryPointArray[i].EntryPointName);
		...
		{
			UE_LOG(LogSkyworthPluginWrapper, Error, TEXT("SkyworthPlugin EntryPoint index:%d could not be loaded: %u"),i, ANSI_TO_TCHAR(entryPointArray[i].EntryPointName));
			result = false;
		}
	}
	...
	if (result)
	{
		UE_LOG(LogSkyworthPluginWrapper, Log, TEXT("SkyworthPlugin initialized successfully"));
	}
	else
	{
		DestroySkyworthPluginWrapper(wrapper);
	}

	return result;
}

void DestroySkyworthPluginWrapper(SkyworthPluginWrapper* wrapper)
{
	if (!wrapper->Initialized)
		return;
	...
	UE_LOG(LogSkyworthPluginWrapper, Log, TEXT("SkyworthPlugin destroyed successfully"));
}
...
static void* LoadEntryPoint(void* Handle, const char* EntryPointName)
{
	...
	if (ptr == nullptr)
	{
		UE_LOG(LogSkyworthPluginWrapper, Error, TEXT("Unable to load entry point: %s"), ANSI_TO_TCHAR(EntryPointName));
	}
	return ptr;
	...
	if (ptr == nullptr)
	{
		UE_LOG(LogSkyworthPluginWrapper, Error, TEXT("Unable to load entry point: %s, error %s"), ANSI_TO_TCHAR(EntryPointName), ANSI_TO_TCHAR(dlerror()));
	}
	return ptr;
#else
	UE_LOG(LogSkyworthPluginWrapper, Error, TEXT("LoadEntryPoint: Unsupported platform"));
	return nullptr;
#endif
}