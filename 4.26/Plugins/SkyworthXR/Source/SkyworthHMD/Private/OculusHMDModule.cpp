...
#define LOCTEXT_NAMESPACE "FSkyworthHMDModule"

SkyworthPluginWrapper FSkyworthHMDModule::PluginWrapper;
FSkyworthHMDModule::FSkyworthHMDModule()
{
...
void FSkyworthHMDModule::StartupModule()
{
	UE_LOG(LogHMD, Log, TEXT("FSkyworthHMDModule::StartupModule"));
	IHeadMountedDisplayModule::StartupModule();
	FString PluginShaderDir = FPaths::Combine(IPluginManager::Get().FindPlugin(TEXT("SkyworthXR"))->GetBaseDir(), TEXT("Shaders"));
	AddShaderSourceDirectoryMapping(TEXT("/Plugin/SkyworthXR"), PluginShaderDir);
}

void FSkyworthHMDModule::ShutdownModule()
{
#if OCULUS_HMD_SUPPORTED_PLATFORMS
	UE_LOG(LogHMD, Log, TEXT("FSkyworthHMDModule::ShutdownModule"));
	if (PluginWrapper.Initialized)
	{
		DestroySkyworthPluginWrapper(&PluginWrapper);
	}
...
FString FSkyworthHMDModule::GetModuleKeyName() const
{
	return FString(TEXT("SkyworthHMD"));
}

void FSkyworthHMDModule::GetModuleAliases(TArray<FString>& AliasesOut) const
{
...
bool FSkyworthHMDModule::PreInit()
{
	UE_LOG(LogHMD, Log, TEXT("FSkyworthHMDModule::PreInit"));
#if OCULUS_HMD_SUPPORTED_PLATFORMS
	if (!bPreInitCalled)
	{
		bPreInit = false;
		...
		if (FApp::CanEverRender() && SkyworthHMD::IsOculusServiceRunning())
		{
			// Load OVRPlugin
			...
			bool pluginWrapperInited = InitializeSkyworthPluginWrapper(&PluginWrapper);
...
bool FSkyworthHMDModule::IsHMDConnected()
{
	UE_LOG(LogHMD, Log, TEXT("FSkyworthHMDModule::IsHMDConnected"));
#if OCULUS_HMD_SUPPORTED_PLATFORMS
	if (FApp::CanEverRender() && SkyworthHMD::IsOculusHMDConnected())
	{
		return true;
	}
#endif // OCULUS_HMD_SUPPORTED_PLATFORMS
...
uint64 FSkyworthHMDModule::GetGraphicsAdapterLuid()
{
...
FString FSkyworthHMDModule::GetAudioInputDevice()
{
...
FString FSkyworthHMDModule::GetAudioOutputDevice()
{
...
TSharedPtr< class IXRTrackingSystem, ESPMode::ThreadSafe > FSkyworthHMDModule::CreateTrackingSystem()
{
#if OCULUS_HMD_SUPPORTED_PLATFORMS
	if (PreInit())
	{
		SkyworthHMD::FSkyworthHMDPtr OculusHMD = FSceneViewExtensions::NewExtension<SkyworthHMD::FSkyworthHMD>();
...
TSharedPtr< IHeadMountedDisplayVulkanExtensions, ESPMode::ThreadSafe >  FSkyworthHMDModule::GetVulkanExtensions()
{
#if OCULUS_HMD_SUPPORTED_PLATFORMS
	if (PreInit())
	{
		...
		{
			VulkanExtensions = MakeShareable(new SkyworthHMD::FVulkanExtensions);
		}
	}
...
#if OCULUS_HMD_SUPPORTED_PLATFORMS
void* FSkyworthHMDModule::GetOVRPluginHandle()
{
...
bool FSkyworthHMDModule::PoseToOrientationAndPosition(const FQuat& InOrientation, const FVector& InPosition, FQuat& OutOrientation, FVector& OutPosition) const
{
	UE_LOG(LogHMD, Log, TEXT("FSkyworthHMDModule::PoseToOrientationAndPosition"));
	SkyworthHMD::CheckInGameThread();

	SkyworthHMD::FSkyworthHMD* OculusHMD = static_cast<SkyworthHMD::FSkyworthHMD*>(HeadMountedDisplay.Pin().Get());

	if (OculusHMD)
	{
		ovrpPosef InPose;
		InPose.Orientation = SkyworthHMD::ToOvrpQuatf(InOrientation);
		InPose.Position = SkyworthHMD::ToOvrpVector3f(InPosition);
...
void FSkyworthHMDModule::SetGraphicsAdapterLuid(uint64 InLuid)
{
...
IMPLEMENT_MODULE(FSkyworthHMDModule, SkyworthHMD)
