...
class FSkyworthHMDModule : public ISkyworthHMDModule
{
public:
	FSkyworthHMDModule();

	static inline FSkyworthHMDModule& Get()
	{
		return FModuleManager::LoadModuleChecked< FSkyworthHMDModule >("SkyworthHMD");
	}
	...
	{
		USkyworthFunctionLibrary::GetPose(DeviceRotation, DevicePosition, NeckPosition, bUseOrienationForPlayerCamera, bUsePositionForPlayerCamera, PositionScale);
	}
	...
	{
		USkyworthFunctionLibrary::GetRawSensorData(AngularAcceleration, LinearAcceleration, AngularVelocity, LinearVelocity, TimeInSeconds, FSTrackedDeviceType::HMD);
	}

	virtual bool GetUserProfile(struct FSHmdUserProfile& Profile) override
	{
		return USkyworthFunctionLibrary::GetUserProfile(Profile);
	}
	...
	{
		USkyworthFunctionLibrary::SetBaseRotationAndBaseOffsetInMeters(Rotation, BaseOffsetInMeters, Options);
	}

	virtual void GetBaseRotationAndBaseOffsetInMeters(FRotator& OutRotation, FVector& OutBaseOffsetInMeters) override
	{
		USkyworthFunctionLibrary::GetBaseRotationAndBaseOffsetInMeters(OutRotation, OutBaseOffsetInMeters);
	}
	...
	{
		USkyworthFunctionLibrary::SetBaseRotationAndPositionOffset(BaseRot, PosOffset, Options);
	}

	virtual void GetBaseRotationAndPositionOffset(FRotator& OutRot, FVector& OutPosOffset) override
	{
		USkyworthFunctionLibrary::GetBaseRotationAndPositionOffset(OutRot, OutPosOffset);
	}

	virtual class IStereoLayers* GetStereoLayers() override
	{
		return USkyworthFunctionLibrary::GetStereoLayers();
	}
...
#if OCULUS_HMD_SUPPORTED_PLATFORMS
	SKYWORTHHMD_API static void* GetOVRPluginHandle();
	SKYWORTHHMD_API static inline SkyworthPluginWrapper& GetPluginWrapper() { return PluginWrapper; }
...
protected:
	...
	static SkyworthPluginWrapper PluginWrapper;
