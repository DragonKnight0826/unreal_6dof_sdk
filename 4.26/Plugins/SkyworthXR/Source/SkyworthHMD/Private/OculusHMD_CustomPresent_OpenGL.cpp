...
{
public:
	FOpenGLCustomPresent(FSkyworthHMD* InOculusHMD, bool srgbSupport);
...
FOpenGLCustomPresent::FOpenGLCustomPresent(FSkyworthHMD* InOculusHMD, bool srgbSupport) :
...
FTextureRHIRef FOpenGLCustomPresent::CreateTexture_RenderThread(uint32 InSizeX, uint32 InSizeY, EPixelFormat InFormat, FClearValueBinding InBinding, uint32 InNumMips, uint32 InNumSamples, uint32 InNumSamplesTileMem, ERHIResourceType InResourceType, ovrpTextureHandle InTexture, TexFlag InTexCreateFlags)
{
	CheckInRenderThread();
...
FCustomPresent* CreateCustomPresent_OpenGL(FSkyworthHMD* InOculusHMD)
{
#if PLATFORM_ANDROID
