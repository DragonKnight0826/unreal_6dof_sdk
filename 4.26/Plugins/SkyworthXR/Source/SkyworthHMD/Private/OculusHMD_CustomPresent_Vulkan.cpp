...
{
public:
	FVulkanCustomPresent(FSkyworthHMD* InOculusHMD);
	...
	virtual FTextureRHIRef CreateTexture_RenderThread(uint32 InSizeX, uint32 InSizeY, EPixelFormat InFormat, FClearValueBinding InBinding, uint32 InNumMips, uint32 InNumSamples, uint32 InNumSamplesTileMem, ERHIResourceType InResourceType, ovrpTextureHandle InTexture, TexFlag InTexCreateFlags) override;
};


FVulkanCustomPresent::FVulkanCustomPresent(FSkyworthHMD* InOculusHMD) :
...
bool FVulkanCustomPresent::IsUsingCorrectDisplayAdapter() const
{
	...
	if (OVRP_SUCCESS(FSkyworthHMDModule::GetPluginWrapper().GetDisplayAdapterId2(&luid)) &&
		luid &&
...
FTextureRHIRef FVulkanCustomPresent::CreateTexture_RenderThread(uint32 InSizeX, uint32 InSizeY, EPixelFormat InFormat, FClearValueBinding InBinding, uint32 InNumMips, uint32 InNumSamples, uint32 InNumSamplesTileMem, ERHIResourceType InResourceType, ovrpTextureHandle InTexture, TexFlag InTexCreateFlags)
{
	CheckInRenderThread();
...
FCustomPresent* CreateCustomPresent_Vulkan(FSkyworthHMD* InOculusHMD)
{
