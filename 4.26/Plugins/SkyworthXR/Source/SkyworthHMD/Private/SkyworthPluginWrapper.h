...
DECLARE_LOG_CATEGORY_EXTERN(LogSkyworthPluginWrapper, Log, All);

#define OCULUS_DECLARE_ENTRY_POINT(Func)		ovrp_##Func* Func

struct SkyworthPluginWrapper
{
	SkyworthPluginWrapper()
	{
		Reset();
	}
	...
	void Reset()
	{
		memset(this, 0, sizeof(SkyworthPluginWrapper));
	...
	OCULUS_DECLARE_ENTRY_POINT(SetUserDefinedSettings); 
	OCULUS_DECLARE_ENTRY_POINT(GetKeyStatue);
