...
struct sxrEventData_Proximity
{
    float distance;                     //!< Proximity in cm
};

struct sxrEventData_State
{
    uint32_t new_state;          //!< new VRMODE
    uint32_t previous_state;          //!< previous VRMODE
};

typedef union
{
    sxrEventData_Thermal    thermal;
    sxrEventData_Proximity  proximity;
    sxrEventData_State      state;
    uint32_t                data[2];
} sxrEventData;
...
enum sxrControllerQueryType
{
    ...
    kControllerQueryControllerMeta = 9
};
...
namespace sxrControllerTouch {
  enum {
    ...
    SecondaryThumbstick = 0x00000020,
    Any                 = ~None
  };
}
...
//! \brief Set the VR service client mode to Start
SXRP_EXPORT SxrResult sxrSetXrStart();

//! \brief Set the VR service client mode to Start
SXRP_EXPORT SxrResult sxrSetXrStop();

//! \brief Set the VR service client mode to Start
SXRP_EXPORT SxrResult sxrSetXrPause();

//! \brief Set the VR service client mode to Start
SXRP_EXPORT SxrResult sxrSetXrResume();

//! \brief Set the keepNotification flag to true
...
//! \brief Retrieve the qvr service data transform to sxr device coordinates
SXRP_EXPORT SxrResult sxrGetQvrDataTransform(sxrMatrix *pQvrTransform);

//! \brief Retrieve the qvr service data inverse transform from sxr device coordinates
SXRP_EXPORT SxrResult sxrGetQvrDataInverse(sxrMatrix *pQvrInverse);
...
//! \brief Get the current state of all the controller
//! \param state sxrControllerState array, such as: sxrControllerState state[2]
//! \param num sxrControllerState array size
SXRP_EXPORT void sxrControllerGetState2(sxrControllerState* state, int num, float predictedTimeMs);
...
//! \param controllerHandle handle for the controller, 0:left handle, 1:right handle, 2:both handel
//! \param what type of event
//! \param arg1 argument for the event
//! \param arg2 argument for the event
SXRP_EXPORT void sxrControllerSendMessage2(int controllerHandle, int what, ...);
...
//! \brief Check whether the Anchors system is enabled
//! \return True/False for Enabled/Disabled
...
//! \brief Get the cloud data from SLAM
//! \param pNumPoints the number of elements for the data
//! \param pCloudData array of cloud points
//! \return SXR_ERROR_NONE for success; SXR_ERROR_UNKNOWN for any other error
SXRP_EXPORT SxrResult sxrGetPointCloudData(uint32_t *pNumPoints, sxrCloudPoint **pCloudData);

//! \brief Release cloud data from SLAM
//! \return SXR_ERROR_NONE for success; SXR_ERROR_UNKNOWN for any other error
SXRP_EXPORT SxrResult sxrReleasePointCloudData();

//! \brief Start Camera
//! \return SXR_ERROR_NONE for success; SXR_ERROR_UNKNOWN for any other error
SXRP_EXPORT SxrResult svrStartCamera();
