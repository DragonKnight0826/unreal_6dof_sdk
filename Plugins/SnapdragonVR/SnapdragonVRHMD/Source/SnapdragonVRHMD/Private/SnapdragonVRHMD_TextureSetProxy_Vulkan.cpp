//=============================================================================
// FILE: SnapdragonVRHMD_TextureSetProxy_Vulkan.cpp
//
...
class FSnapdragonVRTextureSet_Vulkan : public FVulkanTexture2D, public FSnapdragonVRTextureSet
{
public:
	FSnapdragonVRTextureSet_Vulkan(
	...
	virtual ~FSnapdragonVRTextureSet_Vulkan() override;
...
private:
	FSnapdragonVRTextureSet_Vulkan(const FSnapdragonVRTextureSet_Vulkan &) = delete;
	FSnapdragonVRTextureSet_Vulkan(FSnapdragonVRTextureSet_Vulkan &&) = delete;
	FSnapdragonVRTextureSet_Vulkan &operator=(const FSnapdragonVRTextureSet_Vulkan &) = delete;
...
FSnapdragonVRTextureSet_Vulkan::FSnapdragonVRTextureSet_Vulkan(
	...
	FRHIResourceCreateInfo())
{
	...
	for (int i = 0; i < CVars::GNumSwapchainImages; ++i)
	{
		...
		if (EffectiveSamples > 1)
		{
			...
			Texture->MSAASurface = new FVulkanSurface(*Device, VK_IMAGE_VIEW_TYPE_2D, eFormat,
			...
			Texture->MSAAView.Create(*Device, Texture->MSAASurface->Image, VK_IMAGE_VIEW_TYPE_2D, Texture->MSAASurface->GetFullAspectMask(), Texture->MSAASurface->PixelFormat, Texture->MSAASurface->ViewFormat, 0, 1, 0, 1);
		}
		...
		{
			TextureViews[i].Create(*Device, Image, VK_IMAGE_VIEW_TYPE_2D, VK_IMAGE_ASPECT_COLOR_BIT, eFormat, UEToVkFormat(eFormat, false), 0, 1, 0, 1);
		}
	}
...
void FSnapdragonVRTextureSet_Vulkan::SetNativeResource(const FTextureRHIRef& RenderTargetTexture)
{
...
FSnapdragonVRTextureSet_Vulkan* CreateTextureSetProxy_Vulkan(
	class FVulkanDynamicRHI* InVulkanRHI,
	uint32 InSizeX,
	uint32 InSizeY,
	uint32 InArraySize,
...
{
	FSnapdragonVRTextureSet_Vulkan* TextureSetProxy = new FSnapdragonVRTextureSet_Vulkan(
		InVulkanRHI,
		InSizeX,
		InSizeY,
		InArraySize,
...
void TextureSetProxy_SetupRenderLayer(const FSnapdragonVRTextureSet_Vulkan* TextureSetProxy, GSXRRenderLayer* RenderLayer, const uint32* pFd)
{
...
void TextureSetProxy_SwitchToNextElement(FSnapdragonVRTextureSet_Vulkan* TextureSetProxy)
{
	TextureSetProxy->SwitchToNextElement();
}

VkSemaphore TextureSetProxy_GetSignalSemaphore(const FSnapdragonVRTextureSet_Vulkan* TextureSetProxy)
{
	return TextureSetProxy->GetSemaphore()->GetHandle();
}

VkImage TextureSetProxy_GetBackBufferImage(const FSnapdragonVRTextureSet_Vulkan* TextureSetProxy)
{
...
VkImageView TextureSetProxy_GetBackBufferImageView(const FSnapdragonVRTextureSet_Vulkan* TextureSetProxy)
{
